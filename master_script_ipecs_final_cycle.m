...
if strcmp(fname,'p')==1
    ...
    med_ankle_inter=mean(med_ankle_inter,2); %averaging the ankle coord. system positions of each marker, as they should be completely static regardless
    ...
    for n=1:length(Static.Trajectories.Labeled.Data)
        %creating iPECS coordinate system
        ...
        med_knee_inter(:,n)=T_iPECS(:,:,n)\med_knee_t(:,n); %finding important marker positions in iPECS system
    end
    
    med_knee_inter=mean(med_knee_inter,2); %averaging iPECS coord. system position(s) of important marker(s)
...
for filei=1:length(D) %run for loop for each condition
    ...
    if strcmp(fname,'p')==1
        for n=1:length(data.Trajectories.Labeled.Data)
            ...
            iPECS_post(n,:)=data.Trajectories.Labeled.Data(ip,:,n);
            iPECS_mid(n,:)=(iPECS_med(n,:)+iPECS_lat(n,:))/2;
            lat_knee(n,:)=data.Trajectories.Labeled.Data(kl,:,n);
            ...
            knee_center_lab(n,:)=([lat_knee(n,:) 1]+med_knee_lab(n,:))/2;
        end
